...
import sys
sys.path.append('/home/luben/CFVS')
import torch
...
class CoarseMover(object):
    def __init__(self, model_path='kpts/2022-02-??_??-??', model_name='pointnet2_kpts', checkpoint_name='best_model_e_?.pth', use_cpu=False, out_channel=9):
        '''MODEL LOADING'''
        # exp_dir = os.path.join('/home/hcis/YongZhe/CFVS/mankey/log/', model_path)
        exp_dir = '/home/hcis/YongZhe/PDM/CFVS/mankey/log/' +  model_path
    ...
    def inference_from_pcd(self, points, centroid, m, use_offset=False):
        ...
        with torch.no_grad():
            ...
            if not use_offset:
                return real_kpt_pred, real_kpt_x_pred, real_kpt_y_pred,  dir_pred, rot_mat_pred, confidence
            if use_offset:
...
if __name__ == '__main__':
    add_noise = False
    use_offset = False
    model_path = '/kpts/2024-10-24_03-21'
    ...
    data_root = '/home/hcis/YongZhe/PDM/PDM_dataset_inference/Arrow/processed'
    ...
    for key, value in tqdm(data.items()):
        ...
        real_kpt_pred, real_x, real_y, dir_pred, rot_mat_pred, confidence = mover.inference_from_pcd(pcd, centroid, m, use_offset=use_offset)
        real_kpt_pred = real_kpt_pred.reshape(1, 3)
        ...
        rot_grounf_truth = np.array(data[key]['hole_keypoint_obj_top_pose_in_world'])[:3, :3]
        angle = angle_difference(rot_mat_pred, rot_grounf_truth) 
        angle_list.append(angle)
        # visualize hole keypoint
        visualize_pcd = o3d.geometry.PointCloud()
        visualize_pcd.points = o3d.utility.Vector3dVector(np.concatenate((origin_real_pcd, real_kpt_pred, real_x, real_y), axis=0))
        color = np.concatenate((np.repeat(np.array([[0.9, 0.9, 0.9]]),origin_real_pcd.shape[0], axis=0),
                                np.array([[0, 0, 1]]),
                                np.array([[1, 0, 0]]),
                                np.array([[0, 1, 0]])), axis=0)
