...
def parse_args():
    ...
    parser.add_argument('--data', type=str, default='my', help='which data to process')
    parser.add_argument('--mode', type=str, help='It\'s for [train, inference]')
    parser.add_argument('--type', type=str, help='For [coarse, fine]')
    parser.add_argument('--object', type=str, help='Which object')
...
def main(args):
    ...
    if (os.path.exists('visualize') == False) and (args.visualize == True):
        ...
        os.makedirs('./visualize/pcd')
        os.makedirs('./visualize/kpt')
        os.makedirs('./visualize/seg')
        os.makedirs('./visualize/heatmap')
    os.chdir(cwd)
    ...
    with open(os.path.join(data_root, 'peg_in_hole.yaml'), 'r') as f_r:
        data = yaml.load(f_r, Loader=yaml.FullLoader)
    ...
    for key, value in tqdm.tqdm(list(data.items())):
        ...
        for idx, depth_image_filename in enumerate(depth_image_filename_list):
            ...
            xyz, choose = depth_2_pcd(depth_img, factor, intrinsic_matrix) # pcd in camera coordinate
        ...
        # x_2 = np.dot(x_normal_vector, hole_keypoint_bottom_pos_in_world - [0.0, 0., 0.002])        
        x_1 = np.dot(x_normal_vector, hole_keypoint_top_pos_in_world + hole_keypoint_top_pose_in_world[:3, 0] * 0.03)
        x_2 = np.dot(x_normal_vector, hole_keypoint_top_pos_in_world - hole_keypoint_top_pose_in_world[:3, 0] * 0.03)
        x_value = np.sort([x_1, x_2])
        y_normal_vector = hole_keypoint_top_pose_in_world[:3, 1]
        y_1 = np.dot(y_normal_vector, hole_keypoint_top_pos_in_world + hole_keypoint_top_pose_in_world[:3, 1] * 0.03) # 0.067
        y_2 = np.dot(y_normal_vector, hole_keypoint_top_pos_in_world - hole_keypoint_top_pose_in_world[:3, 1] * 0.03) # 0.067
        y_value = np.sort([y_1, y_2])
        z_normal_vector = hole_keypoint_top_pose_in_world[:3, 2]
        z_1 = np.dot(z_normal_vector, hole_keypoint_top_pos_in_world + hole_keypoint_top_pose_in_world[:3, 2] * 0.03) # 0.067
        z_2 = np.dot(z_normal_vector, hole_keypoint_top_pos_in_world - hole_keypoint_top_pose_in_world[:3, 2] * 0.03) # 0.067
        ...
        if args.offset == 'kpts':
            ...
            hole_keypoint_top_x_pos_in_world = hole_keypoint_top_pos_in_world - hole_x_vec * 0.01
            hole_keypoint_top_y_pos_in_world = hole_keypoint_top_pos_in_world - hole_y_vec * 0.003
        ...
        if args.visualize == True:
            pcd = o3d.geometry.PointCloud()
            pcd.points = o3d.utility.Vector3dVector(concat_xyz_in_world)
            o3d.io.write_point_cloud(os.path.join(visualize_folder_path, 'pcd', 'pcd_' + str(key) + '.ply'), pcd)
            ...
            if args.offset == 'kpts':
                ...
                # trimesh = o3d.geometry.TriangleMesh.create_coordinate_frame(size=500)
                # o3d.visualization.draw_geometries([trimesh, concat_pcd])
                # exit()

                o3d.io.write_point_cloud(os.path.join(visualize_folder_path, 'kpt', 'concat_xyzkpt_' + str(key) + '.ply'), concat_pcd)
                ...
                o3d.io.write_point_cloud(os.path.join(visualize_folder_path, 'seg', 'concat_xyzseg_' + str(key) + '.ply'), concat_pcd)
                ...
                o3d.io.write_point_cloud(os.path.join(visualize_folder_path, 'heatmap', 'concat_xyzheatmap_' + str(key) + '.ply'), concat_pcd)
