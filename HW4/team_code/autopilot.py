...
class AutoPilot(autonomous_agent_local.AutonomousAgent):
    ...
    def setup(self, path_to_conf_file, route_index=None, traffic_manager=None, scenario_name=None):
        ...
        if os.environ.get("SAVE_PATH", None) is not None:
            ...
            string = pathlib.Path("").stem
            
            # T_Junction_1_RouteScenario_28154

            print(scenario_name)
            
            route_index = int(scenario_name.split("_")[-1])
            # scenario_name = scenario_name.split("_")[0]
            
            scenario_name = scenario_name.replace(f"_RouteScenario_{route_index}", "")
            
            string += f'{scenario_name}_{route_index}'
            ...
            (self.save_path / "debug_rgb").mkdir()
        ...
        # self.metric_info = {} # for ben2drive metric 
    ...
    def tick_autopilot(self, input_data):
        ...
        # shape = 960, 540 
        rgb_debug = input_data['rgb_debug'][1][:, :, :3]
    ...
    def _get_control(self, input_data, plant):
        ...
        #         target_speed = max(target_speed, frame_mean_speed+0.5)
        ...
        if (self.step % self.config.data_save_freq == 0) :
            ...
            cv2.imwrite(str(self.save_path / 'debug_rgb' / (f'{frame:04}.jpg')), tick_data['rgb_debug'])
    ...
    def save(self, target_point, next_target_point, steering, throttle, brake, control_brake, target_speed, speed_limit,
        ...
        # save route global 
        dense_global_route = []
        global_route_be_saved = self.remaining_route_original[:100]
        for point in global_route_be_saved:
            dense_global_route.append([point[0], point[1]])
        ...
        data = {
            ...
            'global_target_point':[ target_point[0], target_point[1]],
            'global_target_point_next' :[ next_target_point[0], next_target_point[1]],
            'global_pos': [ self._vehicle.get_location().x, self._vehicle.get_location().y, self._vehicle.get_location().z] ,
            'global_yaw': self._vehicle.get_transform().rotation.yaw,
            'global_route_original' : dense_global_route
        ...
        if ((self.step % self.config.data_save_freq == 0) and (self.save_path is not None) and self.datagen):
            ...
            # save bbox 
        # bbox = self.get_bounding_boxes()
        
        # if ((self.step % self.config.data_save_freq == 0) and (self.save_path is not None) and self.datagen):
        #     boxes_file = self.save_path / 'boxes' / f'{frame:04}.json.gz'
        #     with gzip.open(boxes_file, 'wt', encoding='utf-8') as f:
        #         ujson.dump(bbox, f, indent=4)
    ...
    def destroy(self, results=None):
        ...
        if results is not None and self.save_path is not None:
            with gzip.open(os.path.join(self.save_path, 'results.json.gz'), 'wt', encoding='utf-8') as f:
                ujson.dump(results.__dict__, f, indent=2)
        ...
        #cv2.imwrite(str(self.save_path / 'rgb_front' / (f'{frame:04}.jpg')), tick_data['rgb_front'])
        # create video 
        out = cv2.VideoWriter(str(self.save_path / "video.mp4"), cv2.VideoWriter_fourcc(*'mp4v'), 
                        20,  (960, 540)) 
        file_list = sorted(os.listdir(str(self.save_path / "debug_rgb")))
        for file_name in file_list:
            # print(self.save_path / "debug_rgb" / file_name)
            tmp_img = cv2.imread(str(self.save_path / "debug_rgb" / file_name))
            out.write(tmp_img)
        out.release()
    ...
    def ego_agent_affected_by_red_light(self, ego_vehicle_location, ego_vehicle_speed, distance_to_traffic_light, 
        ...
        for light, center, waypoints in self.list_traffic_lights:
            ...
            for wp in waypoints:
                ...
                self.close_traffic_lights.append([bounding_box, light.state, light.id, affects_ego, light])
    ...
    def ego_agent_affected_by_stop_sign(self, ego_vehicle_location, ego_vehicle_speed, next_stop_sign, target_speed, 
        ...
        for stop_sign in stop_signs:
            ...
            self.close_stop_signs.append([bounding_box_stop_sign, stop_sign.id, affects_ego, stop_sign])
    ...
    def get_bounding_boxes(self):
        ...
        ego_speed = self._vehicle.get_velocity().length() # self._get_forward_speed(transform=ego_transform, velocity=ego_velocity)
        ...
        result = {
            ...
            'cord_bounding_box': cord_bounding_box,
            'global_pos': [ego_transform.location.x, ego_transform.location.y, ego_transform.location.z],
            'global_yaw': ego_rotation.yaw
        }
        ...
        obstacles = self._actors.filter("*static.prop*")
        for obstacle in obstacles:
            actor_name = self.get_actor_display_name(obstacle)
            if not (("Streetbarrier" in actor_name) or ("Trafficwarning" in actor_name) or ("Constructioncone" in actor_name)  or  ("Trafficcone01" in actor_name)):
                continue
        
            if obstacle.get_location().distance(self._vehicle.get_location()) < self.config.bb_save_radius:
                obstacle_transform = obstacle.get_transform()
                obstacle_rotation = obstacle_transform.rotation
                obstacle_matrix = np.array(obstacle_transform.get_matrix())
                obstacle_extent = obstacle.bounding_box.extent
                obstacle_id = obstacle.id
                obstacle_extent_list = [obstacle_extent.x, obstacle_extent.y, obstacle_extent.z]
                yaw = np.deg2rad(obstacle_rotation.yaw)
                
                relative_pos = t_u.get_relative_transform(ego_matrix, obstacle_matrix)  
                distance = np.linalg.norm(relative_pos)
                
                cord_bounding_box = {}
                bbox = obstacle.bounding_box
                verts = [v for v in bbox.get_world_vertices(
                obstacle.get_transform())]
                counter = 0
                for loc in verts:
                    cord_bounding_box["cord_"+str(counter)] = [loc.x, loc.y, loc.z]
                    counter += 1
                ...
                result = {
                    'class': 'obstacle',
                    'extent': obstacle_extent_list,
                    'distance': distance,
                    'id': int(obstacle_id),
                    'matrix': obstacle_transform.get_matrix(),
                    'cord_bounding_box': cord_bounding_box,
                    'global_pos': [obstacle_transform.location.x, obstacle_transform.location.y, obstacle_transform.location.z],
                    'global_yaw': obstacle_rotation.yaw
                }
                results.append(result)
        ...
        for vehicle in vehicles: 
            if vehicle.get_location().distance(self._vehicle.get_location()) < self.config.bb_save_radius:
                if vehicle.id != self._vehicle.id:
                    ...
                    vehicle_speed = vehicle_velocity.length() # self._get_forward_speed(transform=vehicle_transform, velocity=vehicle_velocity)
                    ...
                    result = {
                        ...
                        'cord_bounding_box': cord_bounding_box,
                        'global_pos': [vehicle_transform.location.x, vehicle_transform.location.y, vehicle_transform.location.z],
                        'global_yaw': vehicle_rotation.yaw
                    }
                    results.append(result)
        ...
        for walker in walkers:
            if walker.get_location().distance(self._vehicle.get_location()) < self.config.bb_save_radius:
                ...
                walker_speed = walker_velocity.length() # self._get_forward_speed(transform=walker_transform, velocity=walker_velocity)
                ...
                result = {
                    ...
                    'cord_bounding_box': cord_bounding_box,
                    'global_pos': [walker_transform.location.x, walker_transform.location.y, walker_transform.location.z],
                    'global_yaw': walker_rotation.yaw
                }
                results.append(result)
        ...
        for traffic_light in self.close_traffic_lights:
            ...
            result = {
                ...
                'global_pos': [traffic_light_transform.location.x, traffic_light_transform.location.y, traffic_light_transform.location.z],
                'global_yaw': traffic_light_rotation.yaw
            }
            results.append(result)
        ...
        for stop_sign in self.close_stop_signs:
            ...
            result = {
                ...
                'global_pos': [stop_sign_transform.location.x, stop_sign_transform.location.y, stop_sign_transform.location.z],
                'global_yaw': stop_sign_rotation.yaw
            }
            results.append(result)
