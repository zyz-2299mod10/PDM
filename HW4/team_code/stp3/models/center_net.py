...
class CenterNetHead(nn.Module):
  ...
  def __init__(self, input_channel, factor=4, conf_thr=0.4, box_size=[4, 6], output_factor=1):
    super().__init__()
    self.factor = factor
    self.box_size = box_size
    self.conf_thr = conf_thr
    self.heatmap_head = self._build_head(input_channel, 1 * output_factor)
    self.offset_head = self._build_head(input_channel, 2 * output_factor)

  def _build_head(self, in_channel, out_channel):
    """Build head for each branch."""
    layer = nn.Sequential(
                    nn.Conv2d(in_channel, in_channel, stride=2, kernel_size=3, padding=1),
                    # nn.BatchNorm2d(in_channel),
                    nn.ReLU(inplace=True),
                    nn.Conv2d(in_channel, out_channel, stride=2, kernel_size=1),
            )
    # layer = nn.Sequential(nn.Conv2d(in_channel, in_channel, kernel_size=3, padding=1, stride=2), nn.ReLU(inplace=True),
  ...
  def decode_heatmap(self,
    ...
    """
    b, c, feat_h, feat_w = center_heatmap_pred.shape
    center_heatmap_pred = get_local_maximum(center_heatmap_pred, kernel=kernel)
    batch_scores, batch_index, batch_topk_classes, topk_xs, topk_ys = get_topk_from_heatmap(center_heatmap_pred,
                                                                                                k=10)
    offset = transpose_and_gather_feat(offset_pred, batch_index)
    topk_xs = (topk_xs + offset[..., 0]) * self.factor
    topk_ys = (topk_ys + offset[..., 1]) * self.factor
    
    bev_h, bev_w = feat_h * self.factor, feat_w * self.factor
    outputs = []
    for (topk_x, topk_y, batch_score) in zip(topk_xs, topk_ys, batch_scores):
      output = torch.zeros(bev_h, bev_w, device=center_heatmap_pred.device)
      for (x, y, s) in zip(topk_x, topk_y, batch_score):
        if s >= self.conf_thr:
          x, y = x.item(), y.item()
          if len(self.box_size) == 2:
            min_h, max_h = max(0, x - self.box_size[0]), min(bev_h, x + self.box_size[0] + 1)
            min_w, max_w = max(0, y - self.box_size[1]), min(bev_w, y + self.box_size[1] + 1)  
            output[int(min_h): int(max_h), int(min_w): int(max_w)] = 1
          elif len(self.box_size) == 1:
            rr, cc = disk((x, y), self.box_size[0], shape=(bev_h, bev_w))
            output[rr, cc] = 1
